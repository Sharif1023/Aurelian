...
import { Menu, ShoppingBag, Search, Heart, User, Store, X, ArrowRight, Instagram, Twitter, Facebook, Globe } from 'lucide-react';
...
export function Navbar() {
  ...
  const menuItems = categories.map(category => ({
    ...
    subtitle: storeSettings.categorySubtitles?.[category] || 'Explore our collection'
  }));
      ...
      <header className="fixed top-0 w-full z-50 glass border-b border-outline-variant/10">
        <div className="flex justify-between items-center px-6 h-16 w-full max-w-7xl mx-auto">
          ...
          <Link 
            ...
            style={{ color: storeSettings.brandSettings?.color || 'var(--color-primary)' }}
          >
            {storeSettings.brandSettings?.name || 'AURELIAN'}
          </Link>
          <div className="flex items-center gap-2">
            <Link to="/wishlist" className="hidden md:block p-2 hover:opacity-70 transition-opacity active:scale-95 relative">
              <Heart className="w-6 h-6" />
            </Link>
            <Link to="/cart" className="p-2 hover:opacity-70 transition-opacity active:scale-95 relative">
    ...
    <footer className="w-full pt-24 pb-32 bg-white text-primary flex flex-col items-center px-8 text-center relative border-t border-outline-variant/10">
      <div className="relative z-10 w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-16 text-left mb-20">
        <div className="md:col-span-2 space-y-10">
          ...
          <p className="text-on-surface-variant/70 text-sm max-w-sm font-medium leading-relaxed tracking-tight">
            A global destination for curated men's luxury, architectural silhouettes, and bespoke craftsmanship. Redefining the modern man's wardrobe with timeless elegance since 2024.
          </p>
          <div className="flex gap-6">
            {storeSettings.socialLinks.map((social, i) => (
              <a key={i} href={social.url} target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold uppercase tracking-[0.3em] hover:text-primary/60 transition-colors">
                {social.platform}
              </a>
            ))}
          </div>
        ...
        <div className="space-y-8">
          <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] text-primary/40">The Collection</h4>
          <nav className="flex flex-col gap-5">
            <Link to="/track-order" className="text-xs text-primary hover:text-primary/60 transition-colors font-bold uppercase tracking-widest underline decoration-primary/20 underline-offset-4">Track Order</Link>
            <Link to="/collection?category=Combo" className="text-xs text-primary hover:text-primary/60 transition-colors font-bold uppercase tracking-widest">Combo Offers</Link>
            <Link to="/collection?category=Shirt" className="text-xs text-on-surface-variant hover:text-primary transition-colors font-bold uppercase tracking-widest">Shirts</Link>
            <Link to="/collection?category=Pant" className="text-xs text-on-surface-variant hover:text-primary transition-colors font-bold uppercase tracking-widest">Pants</Link>
            <Link to="/collection?category=Shoes" className="text-xs text-on-surface-variant hover:text-primary transition-colors font-bold uppercase tracking-widest">Shoes</Link>
            <Link to="/collection?category=Accessories" className="text-xs text-on-surface-variant hover:text-primary transition-colors font-bold uppercase tracking-widest">Accessories</Link>
          </nav>
        </div>

        <div className="space-y-8">
          <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] text-primary/40">Client Services</h4>
          <nav className="flex flex-col gap-5">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto pt-16 border-t border-outline-variant/10 flex flex-col md:flex-row justify-between items-center gap-8">
        <p className={cn(
          ...
          © 2024 {storeSettings.brandSettings?.name || 'AURELIAN'} LUXE. CURATED ELEGANCE.
        </p>
      </div>
    </footer>
  );
