...
export default function AdminProducts() {
  ...
  const handleOpenModal = (product?: Product) => {
    if (product) {
      ...
      setFormData({
        ...
        image: product.image,
        extraImages: productAny.extraImages || productAny.images || [],
  ...
  const addExtraImageUrl = () => {
    const imageUrl = prompt('Enter extra image URL:');

    if (!imageUrl?.trim()) return;

    setFormData(prev => ({
      ...prev,
      extraImages: [...prev.extraImages, imageUrl.trim()]
    }));
  };

  const addExtraImageFiles = async (
    ...
    setFormData(prev => ({
      ...prev,
      extraImages: [...prev.extraImages, ...images]
    }));
  ...
  const setExtraImageAsMain = (imageUrl: string, index: number) => {
    setFormData(prev => ({
      ...prev,
      image: imageUrl,
      extraImages: [
        prev.image,
        ...prev.extraImages.filter((_, i) => i !== index)
      ].filter(Boolean)
    }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const cleanExtraImages = formData.extraImages.filter(Boolean);

    const hasSizeChart =
      formData.sizeChart.title.trim() ||
    ...
    const payload: any = {
      ...
      productCode:
        ...
        `ARL-${Math.random().toString(36).substr(2, 4).toUpperCase()}`,
  ...
  const toggleSizeAvailability = (index: number) => {
    ...
    setFormData({
      ...formData,
      sizes: newSizes,
      stock: syncStockFromSizes(newSizes).toString()
    });
  };
  ...
  const addSize = () => {
    ...
    if (size) {
      const newSizes = [
        ...formData.sizes,
        { size, isAvailable: true, quantity: 1 }
      ];
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-12">
        <div>
          ...
          <p className="text-on-surface-variant">
            Manage your atelier inventory and archives.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-low border-b border-outline-variant/10">
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant">
                  Product
                </th>
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant">
                  Category
                ...
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant text-right">
                  Actions
                </th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y divide-outline-variant/5">
              {paginatedProducts.map(product => (
                <tr
                  key={product.id}
                  className="hover:bg-surface-low/50 transition-colors group"
                >
                  ...
                  <td className="px-8 py-6 font-headline font-bold text-sm">
            ...
            <div key={product.id} className="p-6 flex gap-4">
              ...
              <div className="flex-grow flex flex-col justify-between">
                <div>
                  ...
                  <p className="font-headline font-bold mt-2">
                    ৳{product.price.toFixed(2)}
                  </p>
                </div>
            ...
            >
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        onChange={e => {
                          ...
                          setFormData({
                            ...
                            sizes: editingProduct
                              ? formData.sizes
                              : getDefaultSizesForCategory(newCat)
                          });
                        }}
                        className="w-full bg-surface-low border-none rounded-xl px-6 py-4 outline-none focus:ring-1 focus:ring-primary"
                  ...
                  <div className="space-y-2 md:col-span-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-4">
                      <div className="w-full h-40 bg-surface-low rounded-2xl overflow-hidden border border-outline-variant/10">
                        {formData.image ? (
                          <img
                            src={formData.image}
                      ...
                      <div className="relative">
                        ...
                        <button
                          ...
                          className="absolute right-2 top-2 p-2 hover:bg-white rounded-lg transition-colors text-on-surface-variant/40 hover:text-primary"
                        >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      <div>
                        ...
                        <p className="text-[10px] text-on-surface-variant/50 mt-1">
                          Add unlimited gallery images. These will appear as
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-4 py-2 bg-white border border-outline-variant/20 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-surface-low transition-colors"
                        >
                          + URL
                        </button>
                        ...
                        <button
                          ...
                          onClick={() =>
                            document
                              .getElementById('extra-images-upload')
                              ?.click()
                          }
                          className="px-4 py-2 bg-primary text-white rounded-lg text-[10px] font-bold uppercase tracking-widest hover:opacity-90 transition-opacity flex items-center gap-2"
                        >
                    ...
                    {formData.extraImages.length > 0 ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {formData.extraImages.map((img, index) => (
                          <div
                            key={`${img}-${index}`}
                            className="relative group bg-surface-low rounded-2xl overflow-hidden border border-outline-variant/10"
                          >
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                              <button
                                type="button"
                                onClick={() => setExtraImageAsMain(img, index)}
                                className="flex-1 py-2 bg-white text-primary rounded-lg text-[9px] font-bold uppercase tracking-widest"
                              >
                                Main
                              </button>
                              ...
                              <button
                                ...
                                className="w-9 h-9 bg-red-500 text-white rounded-lg flex items-center justify-center"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            </div>
                      ...
                      <div className="p-8 rounded-2xl bg-surface-low text-center text-[10px] text-on-surface-variant/60 italic">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {formData.sizes.map((s, index) => (
                      <div
                        key={index}
                        className="relative group bg-surface-low p-4 rounded-2xl border border-outline-variant/10"
                      >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <input
                            ...
                            value={s.quantity}
                            onChange={e =>
                              updateSizeQuantity(index, e.target.value)
                            }
                  ...
                  {formData.sizes.length === 0 && (
                    <p className="text-[10px] text-on-surface-variant/60 italic">
                      No sizes defined for this product.
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/60">
                      Chart Title (e.g. Panjabi Size Chart)
                    </label>
                  ...
                  <div className="overflow-x-auto border border-outline-variant/10 rounded-2xl">
                    <table className="w-full text-left border-collapse min-w-[500px]">
                      <thead>
                        <tr className="bg-surface-low">
                          {formData.sizeChart.columns.map((col, i) => (
                            <th
                              key={i}
                              className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-on-surface-variant relative group"
                            >
                      ...
                      <tbody className="divide-y divide-outline-variant/5">
                        {formData.sizeChart.rows.map((row, rowIndex) => (
                          <tr key={rowIndex}>
                            {formData.sizeChart.columns.map(
                              (col, colIndex) => (
                    ...
                    {formData.sizeChart.rows.length === 0 && (
                      <div className="p-8 text-center text-[10px] text-on-surface-variant/60 italic">
                        No measurement data added yet. Click "+ Add Row" to
                        start.
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">
                    Product Details (Materials, Care, etc.)
                  </label>
